...
int checkStopwords(int key);

char positive[10000][20];
char negative[10000][20];
char test[10000][20];
char stopWords[400][20];
int posLength=10000;
int negLength=10000;
int stopLength=329;
int testLength;
...
int main ( int argc, char *argv[] )
{
  ...
  if ( my_id == master )
  {
    ...
    for(i=1;i<numprocs;i++)
    {
      ...
      MPI_Ssend(stopWords,400*20,MPI_BYTE,i,125,MPI_COMM_WORLD);
    }
    //function(my_id);
  }
  else
  {
    ...
    MPI_Recv(stopWords,400*20,MPI_BYTE,0,125,MPI_COMM_WORLD,&status);
  }
  for(j=0;j<totTest;j++)
  {
    if(checkStopwords(j)!=1)
    {
      tempPosSum=0;
      tempNegSum=0;
  ...
  if ( my_id == master )
  {
    ...
    if(pos_sum_all>neg_sum_all)
      printf("positive Sentence\n");
    else
      printf("Negative Sentence\n");
      t=clock()-t;
      printf("Total time taken to execute %lf\n",((double)t)/1000000);
  }
  MPI_Finalize ( );
#undef N
}
...
void function(int id)
{
   ...
   printf("\nStop words %d\n",id);
   for(int i=0;i<stopLength-324;i++)
   {
     printf("%s\n",stopWords[i]);
   }
 }
...
void readStop()
{
   ...
   {
       fscanf(fp,"%s",a);
       strcpy(stopWords[i],a);
   }
   fclose(fp);
   //printf("Length of the stopWords file is %d\n",stopLength );
}
...
int checkStopwords(int key)
{
  int i;
  for(i=0;i<stopLength;i++){
    if(strcmp(stopWords[i],test[key]) == 0)
      return 1;
  }
  return 0;
}
